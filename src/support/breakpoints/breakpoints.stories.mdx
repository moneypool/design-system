...
$breakpoints: (
  ...
  // Large screen / desktop (980 + (12 * 2)) <= container + gutters
    <<<<<<< HEAD lg: $width-lg ======= lg: $width-lg,
  >>>>>>> Section / Add Utilities section for misc content.Created new Utilities
    section to include misc. content such as breakpoints,
  spacing,
  etc.Currently only breakpoints section is available.No breaking changes.
) !default;
```
...
Media queries are scoped from each breakpoint and upwards. In the example below, the font size is `28px`.
